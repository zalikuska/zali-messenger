...
### ZaliCoin-карточки (`server/src/coins.rs` + `web/src/interface/zalicoin.js`)

- **В канале ZaliCoin уходит карточкой, в личке — переводом.** Карточка: сервер снимает
  `amount × claims` с баланса на удержание (`coin_gifts`), каждый «Получить» переводит `amount`
  активировавшему, «Отменить» (только отправитель) возвращает **только неактивированные** заряды.
  В личке перевод проходит сразу, а карточка лишь сообщает о нём и сверяется с квитанцией
  `GET /api/coins/transfers/:id` (видна только двум сторонам).
- **Квитанция перевода подтверждает ровно одно сообщение** — то, чей `clientId` клиент заранее
  выбрал и передал в переводе (`cardClientId` → `coin_transactions.card_client_id`). Второе
  сообщение с тем же `client_id` в ту же переписку не пускает `idx_messages_client_scope`, поэтому
  повторно вставленный текст карточки не подтверждается ни в загруженной истории, ни за её
  пределами, и от часов устройства это не зависит. `''` — перевод из кошелька (карточки не было),
  `NULL` — перевод до появления привязки; обе карточки такого перевода — только подделка.
  Не подменять эту проверку сравнением времени: время сообщения в браузерном приёме и в
  локальной копии идёт с часов отправителя.
- **Инвариант: `SUM(coin_balances.balance)` + удержанное = 100 000.** Удержанное не хранится
  отдельной колонкой, а считается как `amount × (total_claims − claimed_count)` у `active`-карточек.
  Проверяется `tests/coin_gifts.rs` после каждой операции.
- **Один заряд на аккаунт и защита от одновременных активаций** — три слоя сразу: вся операция в
  одной `BEGIN IMMEDIATE`-транзакции (в отдельной задаче `run_immediate_tx`, по той же причине, что
  в hash chain), условный `UPDATE … WHERE claimed_count < total_claims` и `PRIMARY KEY (gift_id,
  username)` в `coin_gift_claims`. Тест гоняет 8 одновременных «Получить» на карточке с одним зарядом.
- **В шифротексте едет только текст-ссылка**: `🎁 … \n[zc-gift:<id>]` / `💰 … \n[zc-tx:<id>]`
  (`parseCoinCard`). Суммы, остаток и статус карточка берёт с сервера, поэтому нарисовать поддельную
  можно, получить по ней деньги — нет. Новых полей в архиве нет, так что `core/`, macOS, Windows и
  Android не менялись; старые клиенты видят первую строку обычным текстом.
- **Живые обновления — WS-событие `coin_gift_updated`**, которое нативные шеллы пробрасывают общим
  путём (`dispatchRealtimeEvent`). Клиент принимает состояние только «вперёд» (`isNewerCoinGiftState`):
  иначе опоздавший ответ lookup вернул бы кнопку «Получить» уже исчерпанной карточке.
- **i-я полоска заряда — это i-я запись `claims`** (сервер отдаёт их по `claimed_at`), на этом
  держится подсказка «кому ушло». Анимация заполнения (`coinChargePop`) играет только при *росте*
  числа активаций относительно прошлой отрисовки на этом клиенте — история и перезагрузка заполняют
  полоски молча; перерисовка ленты посреди анимации продолжает её отрицательной задержкой
  (`--zc-pop-delay`), а не запускает заново. Монета ZaliCoin — глиф «Ƶ» (Z с чертой) в
  `coinGiftIcon`, кнопке композера (`index.html`), иконке хаба (`prefs.js`), `.zc-orb` и иконке
  `coin` в `uiIcon` (пункт «Казна»); менять рисунок — во всех пяти.
- Карточку нельзя редактировать (`canEditMessage`), в превью/цитате/уведомлении она заменяется на
  `coinCardSummary`. Удержанное всегда можно вернуть с экрана ZaliCoin («Ваши активные карточки»,
  `GET /api/coins/gifts/mine`), даже если сообщение с карточкой не дошло или удалено.
- Карточку публикует `sendInputMessage({ systemText })` — этот режим не трогает композер: черновик,
  вложения и цитата не уезжают вместе с карточкой. Прежний перевод подменял `msgInput.value` и
  отправлял вместе с уведомлением прикреплённые к черновику файлы.

### Казна сервера (`server/src/treasury.rs` + `web/src/interface/treasury.js`)

- **ZaliCoin может лежать на балансе сервера** (`server_treasuries`). Инвариант эмиссии теперь
  `SUM(coin_balances) + SUM(server_treasuries) + удержанное = 100 000`; `/api/coins/distribution`
  отдаёт казны одной суммой (`treasuries`) — имена чужих приватных серверов в общую статистику не
  попадают. Проверяется `tests/treasury.rs` и `tests/coin_gifts.rs`.
- **Пополнить** казну может любой участник сервера — своими монетами — и тот, кто распоряжается
  казной другого сервера, — её монетами. **Выплачивать** (человеку, в казну другого сервера, всем
  участникам роли или `*` — всем участникам вместе с владельцем) может только право «Казна»:
  владелец, админ или роль с `can_manage_treasury`. Роль `owner` адресатом не бывает — это один
  человек.
- Операция — одна `BEGIN IMMEDIATE`-транзакция через `run_immediate_tx`; списание — условный
  `UPDATE … WHERE balance >= ?`, состав роли читается внутри транзакции, итог `amount × получателей`
  списывается целиком или не списывается вовсе. Повтор `(actor, idempotency_key)` возвращает ту же
  операцию (`200` вместо `201`). Тест гоняет 8 одновременных выплат, на которые хватает одной.
- Имена источника/адресата в `treasury_operations` — **снимок на момент операции**: история и
  раздел «От серверов» не должны терять смысл после переименования или удаления сервера.
- **Удаление сервера возвращает казну владельцу** в той же транзакции (`refund_treasury_to_owner`
  из `delete_server`) — иначе монеты выпали бы из эмиссии навсегда.
- Выплата с сервера человеку пишет строку в `treasury_payouts`: из неё экран ZaliCoin строит раздел
  «От серверов» (`GET /api/coins/server-payouts`). WS: `server_treasury_updated` (остаток — всем
  подключённым участникам сервера) и `coin_server_payout` (получателю); нативные шеллы пробрасывают
  их общим путём, дублировать ничего не надо.
- Вход — ПКМ по аватарке сервера → «Казна», пункт виден всем участникам (настройки и участники —
  по-прежнему только владельцу/админу). Модалка переносится в `<body>`: на телефоне лента серверов
  живёт на экране-списке, где `.main` вместе с модалками уехал за край.
- Попутно чинено: `upsert_server_member` превращал любую кастомную роль в `member`, то есть своей
  роли участнику нельзя было назначить вовсе — ни права роли, ни выплата «всем с ролью» до него не
  доходили.
...
## Уведомления — инварианты, которые нельзя терять

`./scripts/notify_doctor/run.sh` проверяет веб-половину на боевом `ZaliInterface`.
Решение «показывать ли уведомление» принимает веб (`notifyBackgroundMessage` →
`SHOW_NOTIFICATION`), поэтому почти все поломки уведомлений — поломки веба или
того, как нативный слой кормит его историей. Разбор 2026-09-13 нашёл пять причин,
которые накладывались друг на друга и давали «работает через раз»:

- **«Чат на экране» ≠ «пользователь смотрит».** `isDmChatVisible`/`isServerChatVisible`
  — вопрос вёрстки, ими гейтится отрисовка и сброс счётчика. Уведомление гейтится
  `isDmChatAttended`/`isServerChatAttended` (плюс `isAppAttended()`: видимость
  документа и `document.hasFocus()`, в мобильной раскладке — только видимость).
  Раньше открытый чат глушил уведомление, звук и счётчик и при свёрнутом окне.
  Счётчик, накопленный в открытом чате, снимает `clearAttendedConversationUnread()`
  на `visibilitychange`/`focus`.
- **Перезагрузка истории отменяет только загрузку ТОЙ ЖЕ переписки.** Догрузка после
  обрыва WS шлёт `REFRESH_HISTORY`/`LOAD_SERVER_HISTORY` пачкой, по одной на
  контакт и канал. Один токен на все переписки (macOS `WebView.swift`, Android
  `NativeBridge.kt` — там ещё и общий для личных и каналов) пропускал только
  последнюю: сообщения, пришедшие за время обрыва, не давали ни уведомления, ни
  счётчика. Токены — по ключу переписки; вместо отмены число одновременных
  загрузок ограничено гейтом (macOS 2, Android 1, LIFO — открытый пользователем
  чат не ждёт хвост догрузки). Windows запускает загрузки независимо и этим не болел.
- **`loadHistory` устаревает по собеседнику, а не по вызову** (`_historyLoadSeqByPeer`).
  Иначе вызов для второго контакта обрывал разбор первого — а новое у первого лежит
  в конце очереди (история идёт по возрастанию).
- **Разбор истории не ждёт кадров в скрытом документе.** Свёрнутому окну, окну в
  трее и фоновой вкладке `requestAnimationFrame` не дают вовсе: продолжение
  (`scheduleHistorySlice`) страхуется таймером, а в скрытом документе кусок идёт
  без бюджета до конца.
- **macOS: закрытие окна не завершает приложение** (`windowShouldClose` прячет окно,
  клик по уведомлению и Dock возвращают). Выход по красной кнопке выключал уведомления
  целиком. Rust-шелл на macOS (экспериментальный) по-прежнему выходит.
- **Windows: AUMID регистрируется самим приложением** в
  `HKCU\Software\Classes\AppUserModelId\com.zali.messenger`. Без регистрации тост
  неупакованного приложения молча отбрасывается (`Show` возвращает успех), а до
  этого AUMID регистрировал только ярлык установщика — у запускавших `.exe` напрямую
  уведомлений не было. На живой Windows не проверялось.

- **Web Push решается по устройству, а не по «есть ли у пользователя сокет»**
  (`server/src/push.rs::push_suppressed_for`). Раньше пуш уходил, только если у
  пользователя **ноль** живых WS: открытое приложение на Mac, полуоткрытый сокет или
  замороженная фоновая вкладка на телефоне (за неё сетевой стек отвечает на Ping, а JS
  стоит) глушили пуш на всех устройствах. Теперь:
  - подписка хранит `device_id` (тот же, что `X-Zali-Device-ID`);
  - вкладка шлёт по своему сокету `client_presence {attended, deviceId}` при открытии,
    `visibilitychange`, `focus`/`blur` и `pagehide` (`web_push.js`);
  - сервер держит это на соединении (`AppState.push_presence`) вместе со временем
    последнего входящего кадра; «смотрю» старше 60 с не в счёт;
  - пуш не уходит только в подписку устройства, где сейчас смотрят в приложение.
    Подписка без `device_id` (старый клиент) живёт по прежнему правилу.
- **Одно сообщение — одно уведомление.** Живая, но не активная вкладка получает и пуш,
  и своё локальное уведомление. У обоих общий `tag` переписки (`zali:dm:<sender>` /
  `zali:<sid>:<cid>`) и `messageId` в `data`; второе молчит — страница его не
  показывает, service worker переподнимает молча (не показать нельзя: `userVisibleOnly`).
- Клик по уведомлению ведёт в переписку (`notificationclick` → `postMessage` или
