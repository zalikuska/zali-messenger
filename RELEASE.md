...
> **Внимание: `/uploads/:filename` для этого не подходит**, вопреки тому что
> написано в `CLAUDE.md`. Это не статическая раздача файлов, а роут вложений:
...
> `filename`, к которой у пользователя есть доступ. Загруженный туда `.exe`
> отдаёт `401`.
>
...
> и онлайн-установщик тоже — он ходит голым `WinHttp`. Значит `downloadUrl`
> обязан быть **публично доступным по HTTPS без аутентификации**.

Варианты, где размещать артефакт:

1. **GitHub Releases** — ничего не надо менять на сервере, ссылка публична,
   отдаётся по HTTPS. Самый простой путь.
2. **Отдельный публичный роут на сервере** — добавить раздачу каталога релизов
   (например `/releases/:filename` через `ServeDir`, без middleware
   аутентификации). Требует правки сервера, тестов и деплоя. Учтите, что это
   сознательно открытый наружу эндпоинт: класть туда можно только то, что
   предназначено для публичного скачивания.
3. **Любой другой HTTPS-хостинг**, доступный без авторизации.
