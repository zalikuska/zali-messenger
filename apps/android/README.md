...
**shared web UI** (`web/`, bundled by `scripts/bundle_web.py`) in a `WebView` and draws a
...
- The web UI is canonical (`web/src/interface.js` → `python3 scripts/bundle_web.py`).
- This shell owns: the WebView host, the native bottom bar (`MainActivity.kt`),
  ...
  `WebViewStore` (`apps/ios/ZaliMessenger/WebView.swift`) protocol-for-protocol — and
...
(`allowed_origins` in `server/src/lib.rs`, no wildcard) always rejects — every API call
...
./apps/android/build_android_core.sh
```
...
`apps/android/app/src/main/jniLibs/<abi>/`, which Gradle picks up automatically — no
...
cd apps/android
./gradlew assembleDebug        # or open the `apps/android/` folder in Android Studio
