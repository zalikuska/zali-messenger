...
if (-not $SkipBundle) {
    ...
    Invoke-CommandChecked -Command $python.Command -Arguments ($python.Args + @('.\bundle_web.py'))
}

Write-Host 'Building Windows client in release mode...'
Invoke-CommandChecked -Command 'cargo' -Arguments @('build', '--release', '--manifest-path', '.\Windows\Cargo.toml')

$sourceExe = Join-Path $PSScriptRoot 'Windows\target\release\zali_messenger_win.exe'
