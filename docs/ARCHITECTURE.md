...
|---|---|---|---|
| `server/src/main.rs` | The server: HTTP API, WebSocket hub, SQLite (~8000 lines; auth, contacts, servers/channels, messages/attachments) | Rust (Axum) | `cargo run` / `cargo check --manifest-path server/Cargo.toml -p zali_server` |
| `server/src/voice.rs` | Voice call signaling split out of `main.rs`: `VoiceRoom` state, join/leave/route/invite handling, the `voice_*` WebSocket message dispatcher | Rust | part of the `zali_server` binary |
| `server/src/devices.rs` | Device trust / E2E key envelope / cloud vault / history-ticket subsystem split out of `main.rs`: device registration/approval/revocation, `post_key_envelope`/`get_key_envelopes`, `post_vault_event`/`get_vault_events`, history tickets, transparency log | Rust | part of the `zali_server` binary |
| `apps/macos/` | The primary desktop client (SwiftUI + WKWebView shell) | Swift | `swift build --package-path apps/macos`, `./scripts/build_app.sh` |
| `apps/windows/src/native.rs` | The Windows client (WRY/TAO shell). Also builds as an **experimental, non-primary** macOS shell | Rust | `cargo run --manifest-path apps/windows/Cargo.toml`, `scripts/build_windows_app.ps1` |
| `core/` | Shared Rust core (crypto, .zali archive I/O, a tiny command bus) compiled to a static lib and linked into the Swift app via FFI | Rust → C ABI | `cargo build --release` inside `core/`, consumed by `apps/macos/Sources/CoreBridge` |
| `sdk/` | The `.zali` archive format itself. `Rust/` is used by the server and the Windows/Rust-shell client; `Swift/` (`ZaliArc.swift`) is a parallel hand-written mirror used by the Swift client | Rust + Swift | Part of `core`/`apps/windows` builds; `Swift/` compiles directly into the SwiftPM target |
...
- **`web/src/interface.js` (~13,800 lines) and `server/src/main.rs` (~8,000 lines,
  ...
  `loader.js` / `styler.js` / `bootstrap.js` (bundled by `scripts/bundle_web.py`
  ...
  *with* it into the final bundle. Don't assume logic has moved there; check
  `scripts/bundle_web.py`'s `js_files` list for the true build order.
- **Two independent `.zali` archive implementations** (`sdk/Rust`
  and `sdk/Swift/ZaliArc.swift`) must stay bit-for-bit compatible
  (same nonce-counter scheme, same PBKDF2 iteration count). A fix made in one
  needs to be checked against the other.
- **`scripts/bundle_web.py` fans one source file out to five locations**:
  `apps/macos/Sources/ZaliMessenger/Assets.swift` (the one that's actually loaded
  at runtime via `WebAssets.html`), `apps/macos/Sources/ZaliMessenger/Resources/Web/{index.html,app.js,style.css,bridge_protocol.json,native_types.js}`
  ...
  run `python3 scripts/bundle_web.py`.**
...
`server/src/main.rs` has had two subsystems pulled out so far:
- `server/src/voice.rs` — `VoiceRoom` state, join/leave/route/invite, the `voice_*`
  WS dispatcher (~780 lines).
- `server/src/devices.rs` — device trust (register/approve/revoke), E2E key envelope
...
2. Move it verbatim into `server/src/<name>.rs`, add `mod <name>;` + `use
