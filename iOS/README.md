...
web UI** (`Web/src/interface.js`, bundled by `bundle_web.py`) in a `WKWebView`. The
...
- The web UI is canonical. Feature/crypto/network changes go in `Web/src/` and are
  bundled with `python3 bundle_web.py`, exactly as for macOS/Windows.
...
(`allowed_origins` in `src/lib.rs`, no wildcard) always rejects that — every API
...
`WHERE owner=? AND recipient_device_id=?` in `src/lib.rs`). If a device's identity
...
the app bundle; `WebViewStore.loadBundledUI()` loads `Web/index.html` via a file URL.
...
- `API_REQUEST`, `NETWORK_CONFIG`, `SET_KEY`, `SEND_MESSAGE`, and
  ...
  (`macOS/Sources/ZaliMessenger/Views/WebView.swift`, `Services/NetworkService.swift`)
...
Ported near-verbatim from `macOS/Sources/ZaliMessenger/Services/ZaliCore.swift` —
same `zali_bus_dispatch` JSON-in/JSON-out C ABI, same `candidateMessageKeys` scoping
logic. Linked via `ZaliCore.xcframework`, built by **`../build_ios_core.sh`**
...
**Run `../build_ios_core.sh` before the first `xcodegen generate` and after any
