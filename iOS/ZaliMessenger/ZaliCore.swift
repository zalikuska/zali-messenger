...
/// Swift wrapper around the Rust Core crate's C ABI (`zali_bus_dispatch`), linked
/// via `ZaliCore.xcframework` (see `build_ios_core.sh` / `project.yml`). Ported
/// verbatim from `macOS/Sources/ZaliMessenger/Services/ZaliCore.swift` — same FFI
/// surface, same JSON-in/JSON-out bus protocol, so this file should stay in sync
/// with the macOS version if the Core crate's API ever changes.
class ZaliCore {
